...
type Web3ProviderState = Web3Provider | undefined;
type Immutables = {
  factory: string;
  token0: string;
  token1: string;
...
export const useUniswap = () => {
  ...
  const [web3Provider, setWeb3ProviderState] = useState<Web3ProviderState>();

  useEffect(() => {
    setWeb3ProviderState((state) => ({ ...state }));
  }, []);
  ...
  async function liquidityExamples(sender: string, exampleType: number) {
    ...
    const block = await web3Provider.getBlock(provider.getBlockNumber());
